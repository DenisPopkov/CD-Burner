#!/usr/bin/env bash
# Verify CD Burner.app architecture and minimum macOS version.
# Usage: verify_cd_burner_app.sh <CD Burner.app> [--legacy|--modern]
set -euo pipefail

if [[ $# -lt 1 ]]; then
  echo "Usage: $0 <CD Burner.app> [--legacy|--modern]" >&2
  exit 1
fi

APP="$1"
MODE="${2:---modern}"
BIN="$APP/Contents/MacOS/CD Burner"
...
echo "CD Burner.app verification ($MODE)"
