...
CassetteProfile CassetteProfile::forCdDigital()
{
    CassetteProfile p;
    p.formulation = TapeFormulation::TypeII;
    p.recordingDeck = RecordingDeck::Generic;
    p.displayName = "CD Digital";
    p.playEqMicroseconds = 0.0f;
    p.hfHeadroomKhz = 20.0f;
    p.maxIntegratedLUFS = -9.0f;
    p.truePeakCeilingDbfs = -0.1f;
    p.hpfCutoffHz = 20.0f;
    p.sideLowCutHz = 80.0f;
    p.sideHighCutHz = 20000.0f;
    p.hfTamerRatio = 1.0f;
    p.hfTamerMaxRatio = 1.0f;
    p.hfTamerThresholdOffsetDb = 0.0f;
    p.saturationDrive = 1.0f;
    p.biasReductionOnHf = 0.0f;
    p.autoPrepRelief = 0.0f;
    p.recordHfPreEmphasis = false;
    p.recordHfPreEmphasisDb = 0.0f;
    p.recordHfPreEmphasisDb2 = 0.0f;
    p.softwareHxPro = false;
    p.emulateHxPro = false;
    p.applyTapeSaturation = false;
    p.gentleTransientShaping = false;
    p.addTapeHissDither = false;
    p.addMicroWowMasking = false;
    return p;
}
